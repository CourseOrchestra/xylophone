...
public class ExcelToFoConverter extends AbstractExcelConverter {
	...
	private static final float PAPER_A4_HEIGHT_INCHES = 29.7f / CM_PER_INCH;
	...
	public static void main(String[] args) {
		...
		try {
			Document doc = ExcelToFoConverter.process(new File(args[0]));
	...
	protected String createPageMaster(HSSFSheet sheet, String pageMasterName) {
		final float paperHeightIn;
		final float paperWidthIn;

		// TODO: other page formats (taken from getPrintSetup()).
		if (sheet.getPrintSetup().getLandscape()) {
			paperHeightIn = PAPER_A4_WIDTH_INCHES;
			paperWidthIn = PAPER_A4_HEIGHT_INCHES;
		} else {
			paperHeightIn = PAPER_A4_HEIGHT_INCHES;
			paperWidthIn = PAPER_A4_WIDTH_INCHES;
		}

		final double leftMargin = sheet.getMargin(HSSFSheet.LeftMargin);
		final double rightMargin = sheet.getMargin(HSSFSheet.RightMargin);
		final double topMargin = sheet.getMargin(HSSFSheet.TopMargin);
		final double bottomMargin = sheet.getMargin(HSSFSheet.BottomMargin);
	...
	protected boolean processCell(HSSFWorkbook workbook, HSSFCell cell,
		...
		if (!emptyStyle) {
			if (noText) {
				 ...
				 */
				/*
				 * Ivan Ponomarev: 2015.04.17: This might be the case for HTML
				 * browsers, but for XSL-FO insertion of a space into empty cell
	...
	protected void processCellStyle(HSSFWorkbook workbook,
		...
		{
			String textAlign = ExcelToFoUtils
					.getAlign(cellStyle.getAlignment());
		...
		case HSSFCellStyle.VERTICAL_CENTER:
			cellTarget.setAttribute("display-align", "center");
			break;
		case HSSFCellStyle.VERTICAL_BOTTOM:
			cellTarget.setAttribute("display-align", "after");
			break;
		}
	...
	protected float processColumnWidths(HSSFSheet sheet, int maxSheetColumns,
		...
		HSSFFont f = sheet.getWorkbook().getFontAt((short) 0);
		int style = (f.getBold() ? Font.BOLD : 0)
				+ (f.getItalic() ? Font.ITALIC : 0);
		Font afont = new Font("Arial", style, f.getFontHeight() / 20);
		JLabel t = new JLabel();
		FontMetrics fm = t.getFontMetrics(afont);
		// TODO better and more precise machinery to convert column widths to
		// real widths
		double inchesPerChar = fm.stringWidth("0000000000") / 720.0;
		// TODO MAGIC MAGIC MAGIC
		inchesPerChar *= 0.925;
		System.out.printf("IPC: %f%n", inchesPerChar);

		double totalWidth = 0;
		for (int c = 0; c < maxSheetColumns; c++) {
			if (!isOutputHiddenColumns() && sheet.isColumnHidden(c))
				continue;
			final double columnWidthIn = (sheet.getColumnWidth(c) / 256.0)
					* inchesPerChar;
			totalWidth += columnWidthIn;
		}
		System.out.printf("h:%d, v:%d%n", sheet.getPrintSetup()
				.getHResolution(), sheet.getPrintSetup().getVResolution());

		// TODO!!!! EXACT PAPER WIDTH HERE!
		double factor = (totalWidth > PAPER_A4_HEIGHT_INCHES - 0.5) ? (PAPER_A4_HEIGHT_INCHES - 0.5)
				/ totalWidth
				: 1.0;
		// double factor = 1.0;

		System.out.printf("total: %f, w: %f, factor: %f%n", totalWidth,
				PAPER_A4_HEIGHT_INCHES, factor);
		for (int c = 0; c < maxSheetColumns; c++) {
			if (!isOutputHiddenColumns() && sheet.isColumnHidden(c))
				continue;

			final double columnWidthIn = (sheet.getColumnWidth(c) / 256.0)
					* inchesPerChar * factor;
			Element col = foDocumentFacade.createTableColumn();
			// col.setAttribute("column-width", columnWidthIn + "in");
			col.setAttribute("column-width", String.format(Locale.ENGLISH,
					"%.2fpt", columnWidthIn * 72.0));
	...
	protected boolean processSheet(HSSFWorkbook workbook, int sheetIndex) {
		...
		createPageMaster(sheet, pageMasterName);
